...
class FusedBatchMatMulOpTestBase : public OpsTestBase {
 protected:
  struct FusedOpsAndDims {
    ...
    // an an empty tensor.
    std::vector<std::initializer_list<int64_t>> fused_dims;
  ...
  void RunQuantizedBatchMatMulAndDequantize(
      const Tensor& x, const Tensor& y,
      const FusedOpsAndTensors& fused_ops_and_tensors, Tensor* result,
      bool adj_x, bool adj_y) {
    DataType t_dtype = DataTypeToEnum<T>::v();
    ...
    MklTestingUtil::GetQuantizationTensors<T>(x, &x_qtensor, DT_QINT8, "SCALED",
                                              &x_min_tensor, &x_max_tensor);
    MklTestingUtil::GetQuantizationTensors<T>(y, &y_qtensor, DT_QINT8, "SCALED",
                                              &y_min_tensor, &y_max_tensor);
    ...
    TF_EXPECT_OK(NodeDefBuilder("fused_batch_matmul", "_QuantizedBatchMatMul")
                     .Attr("Tdevice_inputs", std::vector<DataType>())
                     .Input(FakeInput())
                     .Input(inputs)
                     .Attr("Tdevice_outputs", std::vector<DataType>())
                     .Attr("Thost_outputs", {t_dtype})
                     .Attr("T1", DT_QINT8)
                     .Attr("T2", DT_QINT8)
                     .Attr("Tout", t_dtype)
                     .Attr("U", t_dtype)
  ...
  void VerifyTensorsNear(const std::initializer_list<int64_t>& x_dims,
                         ...
                         const GraphRunner& run_fused, bool adj_x, bool adj_y,
  ...
  void VerifyFusedBatchMatMul(const std::initializer_list<int64_t> x_dims,
    ...
    VerifyTensorsNear(x_dims, y_dims, fused_ops_and_dims, run_default,
                      run_fused, adj_x, adj_y, atol);
  }

  void VerifyQuantizedBatchMatMul(const std::initializer_list<int64_t> x_dims,
                                  const std::initializer_list<int64_t> y_dims,
                                  const FusedOpsAndDims fused_ops_and_dims,
                                  bool adj_x, bool adj_y) {
    const GraphRunner run_default =
    ...
    const GraphRunner run_quantized =
        [&](const Tensor& x, const Tensor& y,
            const FusedOpsAndTensors& fused_ops_and_tensors, Tensor* result,
            bool adj_x, bool adj_y) {
          this->RunQuantizedBatchMatMulAndDequantize(
              x, y, fused_ops_and_tensors, result, adj_x, adj_y);
        };
    const double atol = 1e-2;
    VerifyTensorsNear(x_dims, y_dims, fused_ops_and_dims, run_default,
                      run_quantized, adj_x, adj_y, atol);
  }
};

template <typename T>
using FusedBatchMatMulOpTest =
    FusedBatchMatMulOpTestBase<T, float>;  // additional parameters are float
...
TYPED_TEST_P(FusedBatchMatMulOpTest, QuantizedNoFusion) {
  ...
  for (const bool adj_x : {false, true})
    for (const bool adj_y : {false, true}) {
      ...
      std::initializer_list<int64_t> y_dims = {5, 4, adj_y ? N : K,
                                               adj_y ? K : N};
      this->VerifyQuantizedBatchMatMul(x_dims, y_dims, {{}, {}}, adj_x, adj_y);
    }
}
...
TYPED_TEST_P(FusedBatchMatMulOpTest, QuantizedMulFusion) {
  ...
  for (const bool adj_x : {false, true})
    for (const bool adj_y : {false, true}) {
      ...
      std::initializer_list<int64_t> y_dims = {5, 4, adj_y ? N : K,
                                               adj_y ? K : N};
      this->VerifyQuantizedBatchMatMul(x_dims, y_dims,
                                       {{"Mul"}, {{} /*scalar multiplicand*/}},
                                       adj_x, adj_y);
    }
}
...
using FusedBatchMatMulDataTypes = ::testing::Types<float, bfloat16>;
