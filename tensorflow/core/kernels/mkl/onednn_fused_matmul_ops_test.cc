...
//    (1) Realnumber (float): The output of _FusedMatMul should be
...
// T: float used as tensor type of the MatMul and fusion operation.
template <typename T>
class FusedMatMulOpsTest : public OpsTestBase {
 ...
 protected:
  ...
  void RunQuantizedMatMul(const Tensor& x, const Tensor& y,
    ...
    for (int i = 0; i < fused_ops.size(); ++i) {
      ...
      if (op == "BiasAdd") {
        ...
        ASSERT_EQ(dtype, fusion_tensors[i].dtype());
        Output arg = ops::Const(root.WithOpName(absl::StrCat("arg", i)),
                                Input::Initializer(fusion_tensors[i]));
        fusion_inputs.push_back({arg.name(), 0, dtype});
        num_fusion_inputs++;
    ...
    DataType out_dtype = requantize ? DT_QINT8 : dtype;
  ...
  // The following test runs with 16 configurations.
  //    (1) input quantization mode : {"MIN_FIRST", "SCALED"}
  //    (2) weight quantization per_channel : {false, true}
  //    (3) output is requantized or dequantized:
  //        false: dequantized
  //        true: requantized
  //    (4) weight matrix is transposed : {false, true}
        ...
        for (string input_quant_mode : {"MIN_FIRST", "SCALED"}) {
          for (bool requantize : requantization_config) {
            VerifyTensorsNear<QuantizedGraphRunner>(
                ...
                transpose_x, transpose_y, atol, input_quant_mode, "SCALED",
                false, per_channel, requantize);
          }
        }
      }
    }
