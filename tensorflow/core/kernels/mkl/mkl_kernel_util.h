...
class MklTestingUtil {
 public:
  ...
  static void GetQuantizationTensors(const Tensor& input, Tensor* output,
    ...
    float f32_min = static_cast<float>(min);
    float f32_max = static_cast<float>(max);
    if (mode == "SCALED") {
      float range = std::max(std::abs(f32_min), std::abs(f32_max));
      f32_min = -range;
      f32_max = range;
    }
    RunMklQuantizeOp(input, min, max, out_type, mode, output);
    min_tensor->flat<float>()(0) = f32_min;
    max_tensor->flat<float>()(0) = f32_max;
  }
};
