...
class FusedConvBiasAddAndHardSwishTest : public GrapplerTest {
 public:
  ...
  void RunTest(const string& add_op, const bool is_depthwise) {
    ...
    auto filter = Placeholder(s.WithOpName("filter"), DType, filter_shape);
    auto bias = Placeholder(s.WithOpName("bias"), DType, bias_shape);
    typedef typename EnumToDataType<DType>::Type DT;
    Tensor three(DType, TensorShape({}));
    Tensor one_sixth(DType, TensorShape({}));
    three.scalar<DT>()() = static_cast<DT>(3.0f);
    one_sixth.scalar<DT>()() = static_cast<DT>(1.0f / 6.0f);
    auto const_three =
        ops::Const(s.WithOpName("add_input"), Input::Initializer(three));
    auto const_one_sixth =
        ops::Const(s.WithOpName("one_sixth"), Input::Initializer(one_sixth));
    ...
    Output add;
    if (add_op == kAddV2Op) {
      add = ops::AddV2(s.WithOpName(add_op), const_three, bias_add);
    } else {
      add = ops::Add(s.WithOpName(add_op), const_three, bias_add);
    }

    auto relu6 = ops::Relu6(s.WithOpName("relu_6"), add);
    auto mul_one_sixth =
        ops::Mul(s.WithOpName("mul_one_sixth"), const_one_sixth, relu6);
    auto mul_output = ops::Mul(s.WithOpName("output"), mul_one_sixth, bias_add);
