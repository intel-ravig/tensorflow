...
Status GuardedPCGRandom::Init(OpKernelConstruction* context) {
  // Grab seed Attrs.
  int64 seed, seed2;
  auto status = context->GetAttr("seed", &seed);
  if (!status.ok()) return status;
...
void GuardedPCGRandom::Init(int64 seed, int64 seed2) {
  CHECK(!initialized_);
  if (seed == 0 && seed2 == 0) {
    // If both seeds are unspecified, use completely random seeds.
    seed = random::New64();
  ...
  mutex_lock lock(mu_);
  generator_ = random::PCGRandom(seed, seed2);
  initialized_ = true;
}

random::PCGRandom GuardedPCGRandom::ReserveSamples256(int64 samples) {
  CHECK(initialized_);
  mutex_lock lock(mu_);
  auto local = generator_;
  generator_.Skip(samples);
