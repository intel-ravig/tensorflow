...
class OptimizeForInferenceTest(test.TestCase):
  ...
  def testConvertPlaceholderToConstant(self):
    ...
    r_node = self.create_node_def(
        "Relu", relu_name, [placeholder_name])
  ...
  def testConvertPlaceholderWithDefaultToConstant(self):
    ...
    ph_node = self.create_node_def("PlaceholderWithDefault", placeholder_name,
                                    [a_constant_name])
    self.set_attr_dtype(ph_node, "dtype", dtypes.bool)
    g_def.node.extend([ph_node])
  ...
  def testFuseDecomposedBatchNorm_MatchAll(self):
    for test_rehape in [True, False]:
      ...
      optimized_graph_def = \
        optimize_for_inference_lib.fuse_decomposed_batch_norm(
            original_graph_def)
